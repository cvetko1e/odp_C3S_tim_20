...
export class CommentController {
  ...
  public constructor(private readonly commentService: ICommentService) {
  ...
  private async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (content.length > 2000) {
        res.status(400).json({ success: false, message: "Content must be 2000 characters or fewer" });
        return;
      }

      const parentId = req.body.parentId != null
        ? Number.parseInt(String(req.body.parentId), 10)
        : null;
